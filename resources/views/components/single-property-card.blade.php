<div class="flex-1 mx-2 shadow-lg relative rounded-md mb-6">
    ...
    <div class="py-20 bg-cover bg-center" style="background-image: url('/uploads/{{ $property->featured_image }}')"></div>
    <div class="p-3">
        <h2 class="leading-0 text-base">{{ $property->name }}</h2>
        <h3 class="text-2xl py-3">{{ $property->dynamicCurrency($property->price) }}</h3>
        <div class="border-t-2">
            <ul class="flex items-center -mx-1 my-4">
                ...
                <li class="px-2 py-1 bg-blue-50 rounded-md text-xs mx-1 shadow-sm">
                    {{ $property->net_sqm }} ft<sup>2</sup></li>
            </ul>
